#!/usr/bin/env python3
#
# Simple Python Script to restore module configuration of an existing edge from a modified backup version of its json config file (saved via dump_config.py) 
#
...
update_edge_module = vco_url + 'configuration/updateConfigurationModule'
...
def main():
    ...
    parser.add_argument("-ed", "--edge", type=str,help="Name of Edge to be dumped",required=True)
    parser.add_argument("-d", "--device", action="store_true", help="Name of Edge to be dumped",required=False)
    parser.add_argument("-q", "--qos", action="store_true", help="Name of Edge to be dumped",required=False)
    parser.add_argument("-f", "--firewall", action="store_true",help="Name of Edge to be dumped",required=False)
    parser.add_argument("-ec", "--edgeconfig", type=str,help="Name of JSON file containing the config to be restored",required=True)
    args = parser.parse_args()
    customer=args.enterprise
    partner=args.partner
    operator=args.operator
    EdgeName=args.edge
    edge_config_file=args.edgeconfig
    ...
    if(eid!=0):
        if args.device:
          print("restoring Device Settings for "+EdgeName)
          file_path = edge_config_file  
        try:
            json_data = read_json_file(file_path)
            moduleId = json_data['id']
            configurationId = json_data['configurationId']
            params = {
            "configurationId": configurationId,
            "_update": {"name":"deviceSettings","data":json_data['data']},
            "enterpriseId": eid,
            "id": moduleId,
            }
            resp = requests.post(update_edge_module, headers=headers, data=json.dumps(params))
            print(resp)
            print(resp.json())
        ...
        if args.qos:
          print("restoring Business Policies for "+EdgeName)
          file_path = edge_config_file  
        try:
            json_data = read_json_file(file_path)
            moduleId = json_data['id']
            ...
            params = {
            "configurationId": configurationId,
            "_update": {"name":"QOS","data":json_data['data']},
            "enterpriseId": eid,
            "id": moduleId,
            }
            resp = requests.post(update_edge_module, headers=headers, data=json.dumps(params))
            ...
            print(f"Error: File '{file_path}' not found.")
        if args.firewall:
          print("restoring Firewall Policies for "+EdgeName)
          file_path = edge_config_file  
        try:
            json_data = read_json_file(file_path)
            moduleId = json_data['id']
            ...
            params = {
            "configurationId": configurationId,
            "_update": {"name":"firewall","data":json_data['data']},
            "enterpriseId": eid,
            "id": moduleId,
            }
            resp = requests.post(update_edge_module, headers=headers, data=json.dumps(params))
