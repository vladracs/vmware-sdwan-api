...
get_profile_configuration = vco_url+'configuration/getConfiguration'
######## VCO FUNCTIONS
...
def main():
    ...
    if(eid!=0):
        ...
        response = requests.post(get_profiles, headers=headers, data=json.dumps(params))
        json_response=response.json()
        for item in json_response:
          if 'name' in item:
            ProfileName=item['name']
            ProfileId=item['id']
            ProfileConfig=str(ProfileName)
            params = {"id": ProfileId,
            "with": "modules",
            "enterpriseId": eid}
            resp2 = requests.post(get_profile_configuration, headers=headers, data=json.dumps(params))
           # print(resp2)
           # print(resp2.json())
            json_file = str(ProfileName)+'-Configuration-'+current_time+'.json'
            with open(json_file, 'w') as json_file:
                json.dump(resp2.json(),json_file)
            #print json.dumps(resp2)
            #Save modules individually
            for module in resp2.json()['modules']:
                if module['name'] == 'deviceSettings':
                    deviceSettingsData = copy.copy(module)
                    deviceSettingsId = module['id']
                    #print deviceSettingsId
                    Module_name = str(ProfileName)+'-deviceSettings-'+current_time+'.json'
                ...
                if module['name'] == 'firewall':
                    ...
                    #print firewallId
                    Module_name = str(ProfileName)+'-firewall-'+current_time+'.json'
                    with open(Module_name, 'w') as outfile:
                ...
                if module['name'] == 'QOS':
                    ...
                    QOSId = module['id']
                    #print QOSId
                    Module_name = str(ProfileName)+'-QOS-'+current_time+'.json'
                    with open(Module_name, 'w') as outfile:
                ...
                if module['name'] == 'WAN':
                    ...
                    WANId = module['id']
                    #print WANId
                    Module_name = str(ProfileName)+'-WAN-'+current_time+'.json'
                    with open(Module_name, 'w') as outfile:
